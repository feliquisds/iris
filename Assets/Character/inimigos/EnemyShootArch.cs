...
public class EnemyShootArch : MonoBehaviour
{
    ...
    void ShootArrow()
    {
        GameObject p1 = Instantiate(projectile, new Vector3(
            (transform.position.x + (sprite.flipX ? (spawnXOffset * -1) - 0.1f : spawnXOffset + 0.1f)),
            (transform.position.y + spawnYOffset), transform.position.z), transform.rotation);
        var rigid1 = p1.GetComponent<Rigidbody2D>();
        rigid1.velocity = new Vector2(sprite.flipX ? (firstSpeed * -1) : firstSpeed, verticalSpeed);

        GameObject p2 = Instantiate(projectile, new Vector3(
            (transform.position.x + (sprite.flipX ? (spawnXOffset * -1) : spawnXOffset)),
            (transform.position.y + spawnYOffset), transform.position.z), transform.rotation);
        var rigid2 = p2.GetComponent<Rigidbody2D>();
        rigid2.velocity = new Vector2(sprite.flipX ? (secondSpeed * -1) : secondSpeed, verticalSpeed);

        GameObject p3 = Instantiate(projectile, new Vector3(
            (transform.position.x + (sprite.flipX ? (spawnXOffset * -1) + 0.1f : spawnXOffset - 0.1f)),
            (transform.position.y + spawnYOffset), transform.position.z), transform.rotation);
        var rigid3 = p3.GetComponent<Rigidbody2D>();
        rigid3.velocity = new Vector2(sprite.flipX ? (thirdSpeed * -1) : thirdSpeed, verticalSpeed);
    }
